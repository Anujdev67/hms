...
public class TestAndScansRepository {
    ...
    public void save(TestAndScans test) {
        if (test.getId() == null) {
            String sql = "INSERT INTO testandscans (type, description, deleted) VALUES (?, ?, ?)";
            jdbcTemplate.update(sql, test.getType(), test.getDescription(), test.isDeleted());
        } else {
            String sql = "UPDATE testandscans SET type = ?, description = ?, deleted = ? WHERE id = ?";
            jdbcTemplate.update(sql, test.getType(), test.getDescription(), test.isDeleted(), test.getId());
        }
    }
